...
public class RecommendationGame implements IRecommendation<ProductGame, String>{
	
	private IProductService productService = new ProductService();
	
	public List<ProductGame> retrieveRecommendationsForClient(int client, int recommendationAmount, HashMap<String,String> options) throws DataException, BusinessException{
		...
		ArrayList<ProductGame> previouslyBoughtProducts = new ArrayList<ProductGame>();
		...
		for(Transaction transaction: clientPreviousTransactions) {
			for(Integer productId:transaction.getProductsId()) {
				previouslyBoughtProducts.add((ProductGame)productService.retrieveProductById(productId));
			}
		}
		ArrayList<Tag> tags = new ArrayList<Tag>();
		//Get every instance of a tag in the client's previously bought Products
		for(ProductGame product:previouslyBoughtProducts) {
		...
		if(platform.equals("")) {
			for(Product product:products) {
				...
				if(!(previouslyBoughtProducts.contains(product))) { //If the client already bought a product it won't recommend that product
					...
					weightProducts.add(new WeightProduct(weight, (ProductGame)product));
				}
			}
		}
		else {
			for(Product product:products) {
				...
				if((!(previouslyBoughtProducts.contains(product))) && platform.toLowerCase().equals(productGame.getPlatform().toLowerCase())) { //If the client already bought a product it won't recommend that product
					int weight = 0;
					for(Tag productTag:product.getTags()) {
						for(WeightTag wTag:weightTags) {
							if(wTag.getTag().getId() == productTag.getId()) {
		...
		ArrayList<ProductGame> recommendedProducts = new ArrayList<ProductGame>();
		for(int i = 0; i < recommendationAmount; i++) {
			//Product currentProduct = weightProducts.poll().getProduct();
			recommendedProducts.add((ProductGame) weightProducts.poll().getProduct());
